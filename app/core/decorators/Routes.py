import inspect
from typing import TypeVar, Callable, Type, Any, get_type_hints, List, Union
...
class Routes:
    ...
    def __call__(self, cls, **kwargs):
        print(cls)
        print(self.router)
        self.__cbv(self.router, cls)
        print(dir(cls))
        return cls

    def __cbv(self, router: APIRouter, cls: Type[T]) -> Type[T]:
        """
        ...
        for route in cbv_routes:
            ...
            self._update_cbv_route_endpoint_signature(cls, route)
            cbv_router.routes.append(route)
        print("Вернули в измененный роутер")
        router.include_router(cbv_router)
        self.router.routes = cbv_router.routes
        return cls

    def _init_cbv(self, cls: Type[Any]) -> None:
        """
    ...
    def _update_cbv_route_endpoint_signature(self, cls: Type[Any], route: Union[Route, WebSocketRoute]) -> None:
        """
